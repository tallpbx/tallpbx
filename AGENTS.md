...
- Publish the SHA-256 checksum of `scripts/bootstrap.sh` in the GitHub release notes for each release, naming the release tag, so administrators can use the verified installation path documented in `INSTALL.md` (the verified download URL and `--ref` must use that same tag).
