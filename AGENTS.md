...
7. **Background UAS Port Collision Prevention**: When transitioning between validation phases in `scripts/pbx-sipp-validate.sh`, existing background UAS processes must be cleanly terminated before starting new listeners on ports 5066 and 5088 to prevent `errno 98 (Address already in use)` port collisions.
...
10. **SIP Registration Expiry During Long Test Suites**: The SIP registration scenario (`tools/sipp/register.xml`) must use a sufficiently long lease (`Expires: 3600`) rather than the default 300s so registrations do not expire mid-test. In multi-phase test runners, re-register endpoints before extended parity phases to prevent `Reason: SIP;cause=806;text="USER_NOT_REGISTERED"`.
