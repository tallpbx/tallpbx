...
1. **Public IP Obfuscation**: In all documentation, public test reports, and committed logs, public IP addresses must be obfuscated to show only the last octet (e.g. `x.x.x.218` or `...YYY`). Never expose complete public IP addresses.
2. **Pest Parallel Flag**: Always run the Pest test suite with `--parallel` (`php artisan test --compact --parallel`). Running tests sequentially takes an excessively long time.
3. **SSH Remote Password Handling**: When invoking `php artisan pbx:load-test:seed` on a remote PBX over SSH, do not use special characters (such as `!`) in the password argument (e.g., use `LoadTest1234`), or quote/escape carefully (`--password=\"...\"`). Bash subshells over SSH strip unescaped exclamation marks, creating credential mismatches that cause `403 Forbidden` on SIP REGISTER.
4. **Multi-Host CSV Synchronization**: When seeding accounts on a remote PBX, the generated `storage/app/load-tests/sipp-users.csv` is written on the PBX filesystem. Before running `scripts/pbx-sipp-validate.sh` with `SKIP_SEED=1` on the local load generator orchestrator, copy the CSV from the PBX to the local generator: `scp root@<PBX_HOST>:/var/www/tallpbx/storage/app/load-tests/sipp-users.csv storage/app/load-tests/sipp-users.csv`.
5. **Load Generator Ingress Firewall Rules (`nftables`)**: When testing outbound gateway routing, media flow playback, or extended scenarios, FreeSWITCH bridges calls to the generator host (UAS ports 5088, 5090, RTP 6000). Stateful firewalls on the generator drop unsolicited inbound UDP by default. Always whitelist the PBX IP on the generator before running tests: `sudo nft add element inet tallpbx_filter whitelist_ips { <PBX_IP> }`.
6. **FreeSWITCH Loopback Channel for Internal Bridges**: FreeSWITCH XML dialplan bridges to internal destinations (such as call forwarding) must use `loopback/${safeDest}/${context}` rather than raw extension strings `{dialplan=XML,context=${context}}2000` to avoid `CHAN_NOT_IMPLEMENTED` failures.
7. **Background UAS Port Collision Prevention**: When transitioning between validation steps in `scripts/pbx-sipp-validate.sh`, existing background UAS processes must be cleanly terminated before starting new listeners on ports 5066 and 5088 to prevent `errno 98 (Address already in use)` port collisions.
8. **UAC Port Allocation in Multi-Role Generators**: When the load generator host also runs FreeSWITCH, UAC scenarios must avoid ports 5060 (Sofia internal), 5080 (Sofia external), and 5088 (gateway UAS). `EXTENDED_UAC_LOCAL_PORT` defaults to `5100` (spanning 5100–5114) to prevent socket binding collisions.
9. **PHP-FPM Static Pool Sizing**: Debian default dynamic pool (`pm.max_children = 5`) saturates at concurrency 25, creating worker starvation and latency spikes. For a standard 4GB PBX, configure `/etc/php/8.5/fpm/pool.d/www.conf` to `pm = static` with `pm.max_children = 12` (+15% throughput, 0 errors, 2.9 GiB free RAM).
10. **SIP Registration Expiry During Long Test Suites**: The SIP registration scenario (`tools/sipp/register.xml`) must use a sufficiently long lease (`Expires: 3600`) rather than the default 300s so registrations do not expire mid-test. In multi-scenario test runners, re-register endpoints before extended parity scenarios to prevent `Reason: SIP;cause=806;text="USER_NOT_REGISTERED"`.
...
- Core Project Philosophy & Audience Duality ("Modern simplicity on the surface, enterprise-grade telecommunications engineering under the hood"): TallPBX is built to welcome non-technical office managers while deeply satisfying experienced VoIP engineers and telecom veterans. Never force raw technical complexity onto the surface, and never dumb down the underlying telephony engine. Maintain progressive depth: present clean, intuitive headline summaries by default (e.g. Average, Min, Max), placing advanced technical depth (percentiles, packet dumps, FreeSWITCH channel variables) inside collapsible details blocks or linked deep-dive guides. When communicating this duality across user-facing pages, adapt the phrasing naturally to the specific context rather than repeating an identical marketing line across multiple documents (the exact phrase "Modern simplicity on the surface, enterprise-grade telecommunications engineering under the hood." is reserved exclusively for the Guest Landing Page).
- Target Audience & Plain-Language Standards: The primary audience consists of PBX and office administrators who may or may not be technical telephony or Linux experts. All user-facing text — interactive installer prompts, Artisan CLI commands, web panel labels, form helper text, tooltips, validation errors, and documentation — must be descriptive, friendly, and intuitive without being overly technical, jargon-heavy, or wordy. Explain the practical purpose of a feature or setting in simple terms (e.g., explaining that voice prompts are "spoken recordings for voicemail, call menus, and system messages" rather than referring to "say grammar modules"), and always provide a clearly designated recommended default.
...
- XML handler successful request logging is intentionally opt-in through `FREESWITCH_XML_HANDLER_LOG_REQUESTS`; avoid enabling it during load tests. Redis is the recommended default cache and session store for this project (`CACHE_STORE=redis`, `SESSION_DRIVER=redis`, `SESSION_CONNECTION=cache`). The telephony XML cache is controlled by `XML_CACHE_TTL` and `XML_CACHE_STORE` (with granular subsystem overrides); avoid file/database stores for concurrency tests because they add their own latency. First-party context-wide contributor fragments are controlled by `XML_CACHE_CONTRIBUTOR_TTL`. Keep `mod_hiredis` installed/loaded by default, but keep generated per-call hiredis dialplan behavior opt-in through `FREESWITCH_HIREDIS_DIALPLAN_LIMIT_ENABLED` and `FREESWITCH_HIREDIS_DIALPLAN_MARKER_ENABLED`.
- Use `--label`, `--max-failure-rate`, and `--max-average-ms` on `pbx:load-test:dialplan` when comparing repeat runs. The JSON report is the durable source for run ID, Git state, environment, target scenario mix, success/failure rates, average, min, and max latencies, thresholds, and pass/fail reasons.
- For SIPp end-to-end validation, prefer running `scripts/pbx-sipp-validate.sh` from a separate Linux host or VM against the PBX host. The runner registers seeded users, starts auto-answer UAS scenarios, places extension-to-extension and outbound-route calls, and stores artifacts under `storage/app/load-tests/sipp-e2e-*`. Set `MEDIA_FLOW=1` only when live recording/MOH/announcement RTP checks are needed; it seeds synthetic media destinations and uses SIPp RTP echo with optional tcpdump pcaps. Use `docs/load-testing-guide.md` as the authoritative operating guide for topology, manual commands, and artifact interpretation, and `docs/load-testing-results.md` for empirical benchmark results and hardware sizing. If `sipp` is missing, install it on the load generator before running the script.
