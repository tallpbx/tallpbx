...
#### Task 3 live validation results (2026-08-30, disposable server 209.38.148.161, commit 67571dc; F6b closed 2026-08-31)
...
| Voicemail app registration | **PASS** | Deposit lands in the managed root (`store/runtime/1/voicemail-message/1000/msg_<uuid>.wav`), nothing left in `/var/lib/freeswitch/storage`; listener registered the message (row `01a0587c-...`, `freeswitch_domain=209.38.148.161`, uuid matches fsdb id) — see F6 |
...
On the disposable Debian 13 VPS (209.38.148.161, production mode):
...
3. **Seed**: `pbx:load-test:seed` into the Default tenant (slug `default`, realm 209.38.148.161 — reuses the seeded realm row, avoids domain ambiguity) with 20 extensions (2000–2019), media + extended fixtures; CSV `storage/app/load-tests/sipp-users-vps.csv` (SEQUENTIAL header, realm-matched).
