...
10. **Hardened Linux CLI Execution & Jailbreak Defense (Bounded Sudoers Architecture)**:
    ...
    - Web application processes run under the unprivileged `www-data` user. Privileged host mutations are strictly encapsulated in a dedicated root-owned helper script (`/usr/local/bin/tallpbx-security`, mode `0750 root:www-data`), with matching sudoers drop-in `/etc/sudoers.d/tallpbx-security`.
