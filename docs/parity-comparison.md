...
| **Theme & Dark Mode Support** | Core / `admin` (instant switchable Light, Dark, and System theme with zero-flicker local caching & per-user database persistence) | Static light theme (manual CSS edits/skins required; no native dynamic dark mode toggle or system sync) | Static light theme (classic FreePBX skin; no dark mode or system auto-detection) | 🚀 **Superior in TallPBX** |
...
6. **Modern UI with Dual Layout Modes, Collapsible Navigation & Switchable Themes (Light, Dark, System)**:
   - TallPBX gives operators flexible control over their workspace layout and visual ergonomics. Administrators can collapse the vertical sidebar into an icon-only mini rail (`64px`) to maximize table width for dense views (CDRs, routing rules, active calls), or switch to a horizontal top-bar header with standard dropdown menus for users accustomed to legacy PBX navigation. Layout preferences and sidebar sizes are saved per-user and rendered with zero layout shift.
   - **Switchable Light, Dark, and System Themes**: Operators can switch instantly between high-contrast Light mode, low-glare Dark mode, and automatic System preference matching (`prefers-color-scheme`).
   - **Zero-Flicker Architecture & Ergonomics**: Theme choices are committed synchronously to browser `localStorage` and executed by an inline `<head>` script prior to DOM painting, eliminating the white flash-of-unstyled-theme (FOUC) during page refreshes and Livewire navigation transitions, while remaining asynchronously synchronized with user database profiles. Dark theme significantly reduces eye fatigue for telecom engineers and dispatchers working overnight shifts in 24/7 Network Operations Centers (NOCs) and low-light control rooms, while Light theme delivers sharp readability for brightly lit daytime offices.
   - **Competitive Comparison**: FusionPBX relies on legacy static CSS stylesheets embedded in procedural PHP templates; modifying appearance requires administrative server file edits or custom theme packages, with no user-level runtime switching or OS dark mode integration. FreePBX features a static, single-theme light skin with no native dark mode support in core. TallPBX is the only platform among the three to offer seamless, zero-flicker per-user theme and layout customizability out of the box.
...
9. **Native Kernel Firewall (`nftables`) & Multi-Vector Intrusion Defense with Zero-Lockout**:
   ...
   - All-or-nothing (atomic) preflight verification compiles rules to `/etc/tallpbx/firewall.nft.pending` and tests them with `nft -c -f` before replacing the active ruleset, preventing invalid syntax from ever locking out the administrator.
