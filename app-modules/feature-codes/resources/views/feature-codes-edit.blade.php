<div>
    <div class="mb-6">
        <h2 class="text-2xl font-semibold">
            {{ $this->isEdit ? __('client.edit') : __('client.create') }} {{ __('admin.feature_code') }}
        </h2>
    </div>
        ...
        <div class="card-body">
            <form wire:submit="save" class="space-y-4">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div class="form-control w-full">
                        <label class="label justify-start gap-2 pb-1"><span class="label-text font-medium">Code</span></label>
                        <input type="text" wire:model="code"
                               class="input input-bordered w-full font-mono @error('code') input-error @enderror"
                               placeholder="*97" />
                        @error('code') <span class="label-text-alt text-error mt-1">{{ $message }}</span> @enderror
                    </div>
                </div>
                ...
                <div class="flex gap-2 pt-4 border-t border-base-300">
                    ...
                    <a href="{{ route('panel.feature-codes.index') }}" class="btn btn-ghost">{{ __('client.cancel') }}</a>
                </div>
            </form>
        </div>
    </div>
